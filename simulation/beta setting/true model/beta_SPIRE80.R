...
for (i in 1:1000) {
  ...
  m <- 20
  x.a <- seq(from=0, to=0.9999, length.out=m)
  ...
  b.cz <- function(beta,c,x.a,z1,z2){
    p <- dbeta(x.a,1.6+5*c,2+z1+z2)/sum(dbeta(x.a,1.6+5*c,2+z1+z2))
  ...
  spire.func <- function(beta,y,w,delta,z1,z2,x.a){
    if(delta==0){
      p <- dbeta(x.a,1.6+5*w,2+z1+z2)/sum(dbeta(x.a,1.6+5*w,2+z1+z2))
