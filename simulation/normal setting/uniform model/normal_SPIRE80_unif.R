...
for (i in 1:1000) {
  ...
  S.beta <- function(beta,y,w,delta,z){
    if(delta==0){
      denom <- sum(f.y.xz(beta,y,x.a,z)*(x.a>w)*p)
      res1 <- sum(S.beta.f1(beta,y,x.a,z)*f.y.xz(beta,y,x.a,z)*(x.a>w)*p)/denom
      res2 <- sum(S.beta.f2(beta,y,x.a,z)*f.y.xz(beta,y,x.a,z)*(x.a>w)*p)/denom
      res3 <- sum(S.beta.f3(beta,y,x.a,z)*f.y.xz(beta,y,x.a,z)*(x.a>w)*p)/denom
      c(res1,res2,res3)
    }else{
      S.beta.f(beta,y,w,z)
    }}
